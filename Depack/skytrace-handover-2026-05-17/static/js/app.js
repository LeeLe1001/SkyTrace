...
// ==================== 🆕 Flighty 化组件渲染 (Phase 5) ====================
// 以下函数使用 flighty-theme.css 中的 Flighty 设计系统渲染 UI 组件

/** 渲染 Flighty 风格航班卡片 */
function renderFlightyFlightCard(flight) {
    const logoUrl = flight.airline
        ? `/static/img/airlines/${flight.airline.toLowerCase()}.png`
        : '/static/img/airlines/default.png';
    const statusClass = {
        'scheduled': 'on-time', 'checkin': 'boarding', 'boarding': 'boarding',
        'in-flight': 'in-flight', 'completed': 'landed',
    ...
    <div class="flight-card" data-flight-id="${flight.id}" onclick="openFlightDetail('${flight.id}')">
        ...
        <div class="flight-info">
            ...
            <span class="flight-route">
                ${flight.departure || '???'}<span class="arrow">→</span>${flight.arrival || '???'}
            </span>
        </div>
...
function renderStatusBadge(status, text) {
    const classMap = {
        'scheduled': 'on-time', 'checkin': 'boarding', 'boarding': 'boarding',
        'in-flight': 'in-flight', 'completed': 'landed',
        'delayed': 'delayed', 'canceled': 'canceled', 'diverted': 'diverted',
    };
    const cls = classMap[status] || 'on-time';
    return `<span class="status-badge ${cls}"><span class="dot"></span>${text || status}</span>`;
}

/** 渲染倒计时组件 */
function renderFlightyCountdown(flight) {
...
function refreshFlightCards() {
    ...
    if (!container) return;
    const allFlights = flightStore ? flightStore.get('flights') : flights;
    const filter = uiStore ? uiStore.get('statusFilter') : 'upcoming';
    let filtered;
    if (filter === 'upcoming')
    ...
    if (filtered.length === 0) {
        container.innerHTML = `<div class="empty-state"><div class="icon">✈️</div><h3>${t ? t('emptyTrips') : '暂无行程'}</h3><p>${t ? t('emptyHint') : '点击右下角 + 按钮添加您的第一个航班'}</p></div>`;
    } else {
        container.innerHTML = filtered.map(renderFlightyFlightCard).join('');
    }
}

// ==================== 🆕 Lottie 雷达动画集成 ====================
(function initFlightyFeatures() {
    // 初始化 Lottie 动画
    if (typeof LottieUtils !== 'undefined') LottieUtils.init();

    // 在航班查询时显示雷达动画
    const origLookup = lookupFlight;
    if (origLookup) {
        lookupFlight = async function() {
            if (typeof LottieUtils !== 'undefined') LottieUtils.show();
            try { return await origLookup.apply(this, arguments); }
            finally {
                if (typeof LottieUtils !== 'undefined') LottieUtils.hide();
            }
        };
    }

    // 数据变化时同步到 Store
    if (typeof flightStore !== 'undefined') {
        const origLoadFlights = loadFlights;
        loadFlights = async function() {
            await origLoadFlights.apply(this, arguments);
            flightStore.set('flights', flights);
        };
    }
