...
| 层 | 技术 | 说明 |
|---|------|------|
| **后端** | Python 3.8+ / Flask | RESTful API，无数据库，纯 JSON 文件存储 |
...
### v20 (2026-02-18)
- 🌡️ 热力图开启时隐藏航线（纯热力图模式）
- 🌡️ 修复热力图全蓝色问题（sqrt 归一化）
- 🔵 修复反子午线机场圆点消失（lon±360 镜像标记）
- 📦 版本 20

### v19 (2026-02-18)
- 🌡️ 新增行程地图热力图功能（Leaflet.heat 插件）
- 🔵 反子午线机场标记修复（L.circleMarker → L.marker + L.divIcon）
- 🌐 修复语言切换后首页卡片显示错误语言

### v18 (2026-02-17)
- 🗺️ 全屏地图修复（opacity 动画冲突）
- 🗺️ 首页地图语言切换后消失修复
- 🔗 联程解绑按钮修复 / 跨日联程布局优化
- ⏱️ 飞行时长 UTC 计算修复
...
### v17 (2026-02-16)
- 🔗 联程航班轮播/断开/连接模式修复
- 📱 全屏地图 / i18n 补全 / 断开按钮全局委托

### v16 (2026-02-15)
- 🏗️ UI 重构：高级筛选、表单、详情页、机场 i18n
- 🔧 修复 9 项 UI 问题
